...
export class Game{
    ...
    undo(){
        ...
        // 获取撤销快照并恢复数据
        const snapshot = this._history.popUndo();
        const {row, col, oldValue} = snapshot;
        
        // 不可变更新棋盘
        const restoredSudoku = this._currentSudoku.guess({row, col, value: oldValue});

        this._currentSudoku = restoredSudoku;
    }

    /**
     * 重做上一步撤销的操作
     * 基于二叉树的活跃子节点，回到最近的前进状态
     * @returns {void}
     */
    redo(){
        if(!this.canRedo()){
        ...
        // 获取重做快照并恢复数据
        const snapshot = this._history.popRedo();
        const {row, col, newValue} = snapshot;

        // 不可变更新棋盘
        const restoredSudoku = this._currentSudoku.guess({row, col, value: newValue});
