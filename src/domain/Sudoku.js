...
export class Sudoku{
   ...
   * @constraint 外部传入的 grid 必须是合法 9x9 结构（当前由工厂函数保证）
   */
  constructor(grid, question){
    // 归一化
    const normalized = grid.map(row => row.map(cell => cell == null ? 0 : cell));

    this.#grid = normalized;

    // 如果传入了 question，用它；否则用 grid
    this.#question = question ? question.map(row => row.map(c => c == null ? 0 : c)): normalized;
  }
